...
GLboolean TriangulatedMesh3::SaveToOFF(const std::string& file_name) const
{
    ...
    f << VertexCount() << " " << FaceCount() << " " << 0 <<endl;
...
ostream& cagd::operator <<(std::ostream& lhs, const TriangulatedMesh3& rhs)
{
...
istream& cagd::operator >>(std::istream& lhs, TriangulatedMesh3& rhs)
{
    ...
    rhs._vertex.resize(n_vertex);
    rhs._face.resize(n_face);
    rhs._normal.resize(n_vertex);
    rhs._tex.resize(n_vertex);
