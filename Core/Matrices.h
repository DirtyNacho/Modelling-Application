...
namespace cagd
{
    ...
    GLboolean Matrix<T>::SetColumn(GLuint index, const ColumnMatrix<T>& column)
    {
        ...
        for(unsigned int row=0;row<_row_count;++row)
        {
            _data[row][index]=column._data[row][0];
        }
    ...
    inline RowMatrix<T>::RowMatrix(GLuint column_count):
        Matrix<T>::Matrix(1,column_count) {}
    ...
    inline GLboolean TriangularMatrix<T>::ResizeRows(GLuint row_count)
    {
        ...
        for(unsigned int row=_row_count;row<row_count;row++)
        {
            _data[row].resize(row+1);
        }
        _row_count= row_count;
